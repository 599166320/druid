...
import org.apache.druid.query.topn.*;
...
public class DruidQuery1
{
  ...
  public static DruidQuery1 fromPartialQuery(
    ...
    if (sorting != null) {
      ...
      if (sortKind == Sorting.SortKind.UNORDERED) {
        ...
        if(sorting.limitSpec() != null && sorting.limitSpec() instanceof DefaultLimitSpec){
          ...
          List<String> orderByColumn = limitSpec.getColumns().stream().map(d->d.getDimension()).collect(Collectors.toList());
          ...
          plannerContext.getQueryContext().put("orderByColumn",orderByColumn);
