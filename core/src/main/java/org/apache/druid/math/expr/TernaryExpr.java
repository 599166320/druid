...
class TernaryExpr implements Expr
{
  ...
  {
    return logicalExpr.analyzeInputs()
                      .with(firstExpr)
                      .with(secondExpr)
                      .withScalarArguments(ImmutableSet.of(logicalExpr, firstExpr, secondExpr));
  }
  ...
  {
    ExpressionType firstExprType = firstExpr.getOutputType(inspector);
    ExpressionType secondExprType = secondExpr.getOutputType(inspector);
    if (!firstExprType.is(secondExprType.getType())) {
